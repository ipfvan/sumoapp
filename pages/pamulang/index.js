...
import HeaderLima from './sidebar/cibinong/main-menu/HeaderLima';
import HeaderEnam from './sidebar/cibinong/logistik/HeaderEnam';
...
export default function PamulangHome() {
  ...
  const loadNotifications = async () => {
    try {
      const response = await axios.get('/api/pamulang/main/notifications');
  ...
  const clearNotifications = async () => {
    try {
      await axios.delete('/api/pamulang/main/notifications');
      setNotifications([]);
  ...
  const toggleNotifications = async () => {
    ...
    if (isNewNotification && notifications.length > 0) {
      try {
        ...
        await axios.put('/api/pamulang/main/notifications/', { notificationIds });
  ...
  useEffect(() => {
    ...
    const authenticatedpamulang = document.cookie.includes('authenticatedpamulang=true');

    // Jika pengguna tidak otentikasi, redirect ke halaman login
    if (!authenticatedpamulang) {
    router.push('/pamulang/login');
    }
  }, []);
