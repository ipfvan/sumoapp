...
import Pamulang from './main/Pamulang';
...
const Sidebar = ({ activeMenu, setActiveMenu, sidebarOpen, setSidebarOpen }) => {
  ...
  const branches = ['Pamulang', 'Cibinong', 'Ciledug' ];
  const [selectedBranch, setSelectedBranch] = useState('Pamulang');

  useEffect(() => {
    if (selectedBranch === 'Pamulang') {
