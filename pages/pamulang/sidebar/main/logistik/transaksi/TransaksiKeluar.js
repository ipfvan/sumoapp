...
const TransaksiKeluar = () => {
  ...
  const fetchFilteredData = async () => {
    try {
      const response = await axios.get(`/api/pamulang/main/transaksi-keluar`, {
        params: {
  ...
  const handleSubmit = (event) => {
    ...
    if (selectedKategori && selectedItem && tanggalTransaksi && qty && penerima) {
      ...
      axios.post('/api/pamulang/main/transaksi-keluar', data)
        .then(response => {
          ...
          if (penerima === 'Ciledug') {
            ...
            axios.post('/api/pamulang/ciledug/submit-transaksi-masuk-ciledug', transaksiMasukData)
              .then(response => {
            ...
            axios.post('/api/pamulang/cibinong/submit-transaksi-masuk-pamulang', transaksiMasukData)
              .then(response => {
    ...
    if (!result.destination) {
      ...
      if (itemToDelete) {
        if (window.confirm(`Are you sure you want to delete ${findItemNameById(parseInt(itemToDelete.barang_id), bgst)}?`)) {
          ...
          axios.delete(`/api/pamulang/main/transaksi-keluar?id=${itemId}`)
            .then(response => {
              ...
              if (itemToDelete.penerima === 'Ciledug') {
                ...
                const deleteParams = {
                  ...
                  pengirim: 'Pamulang' // Ubah sesuai dengan pengirim yang diinginkan
                };
  
                // Perform delete operation using API untuk menghapus data berdasarkan tanggal_transaksi, barang_id, qty, dan pengirim
                axios.delete('/api/pamulang/ciledug/delete-transaksi-masuk-ciledug', { params: deleteParams })
              ...
              if (itemToDelete.penerima === 'Cibinong') {
                ...
                // Perform delete operation using API untuk menghapus data berdasarkan tanggal_transaksi, barang_id, qty, dan pengirim
                axios.delete('/api/pamulang/cibinong/delete-transaksi-masuk-pamulang', { params: deleteParams })
                  .then(deleteResponse => { // Data berhasil dihapus, lakukan apa yang perlu dilakukan
                    console.log('Data from cibinong_db deleted:', deleteResponse.data);
                  })
                  .catch(deleteError => {
                    console.error('Error deleting data from cibinong_db:', deleteError);
                  });
  ...
  const fetchDetailedData = async () => {
    try {
      const response = await axios.get(`/api/pamulang/main/get-detailed-transaksi-keluar`, {
        params: {
          startDate: selectedStartDate,
          endDate: selectedEndDate,
        },
  ...
  const handleSimplifyClick = () => {
    ...
    axios.delete('/api/pamulang/main/simplify-transaksi-keluar')
      .then(response => {
