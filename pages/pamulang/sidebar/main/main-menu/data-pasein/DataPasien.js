...
const handleEditSubmit = async (e) => {
  ...
  try {
    ...
    for (const caseItem of cases) {
      ...
      const transaksiKeluarResponse = await axios.get(`/api/pamulang/main/transaksikeluar?id=${modifiedId}`);
      ...
      if (transaksiKeluarResponse.data.length === 0) {
        const postResponse = await axios.post(`/api/pamulang/main/transaksikeluar`, postData);
        console.log(postResponse.data.message);
      } else {
        const updateResponse = await axios.put(`/api/pamulang/main/transaksikeluar`, {
    ...
    const response = await axios.put(`/api/pamulang/main/pasien?id=${selectedPasien.id}`, {
      ...editData,
      tanggal: isoTanggal,
    });
  ...
  const handleDelete = async (id) => {
    ...
    if (isConfirmed) {
      try {
        const response = await axios.delete(`/api/pamulang/main/pasien?id=${id}`);
        console.log(response.data.message);
  ...
  useEffect(() => {
    const fetchMetodeSunatOptions = async () => {
      try {
        const response = await axios.get('/api/pamulang/main/metode-sunat');
  ...
  useEffect(() => {
    const fetchMetodeBiusOptions = async () => {
      try {
        const response = await axios.get('/api/pamulang/main/metode-bius');
  ...
  const handleDateChange = async (event) => {
    ...
    try {
    const response = await axios.get(`/api/pamulang/main/available-times?tanggal=${selectedDateString}`);
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      await axios.post('/api/pamulang/main/pasien', data);
