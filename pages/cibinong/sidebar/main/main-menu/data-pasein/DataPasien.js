...
export default function DataPasien()  {
  ...
  useEffect(() => {
    const fetchOptions = async (kategori, setOptions) => {
      try {
        const response = await axios.get(`/api/cibinong/main/items-by-category?kategori=${kategori}`);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('/api/cibinong/main/pasien');
...
const handleEditSubmit = async (e) => {
  ...
  try {
    ...
    for (const caseItem of cases) {
      ...
      const transaksiKeluarResponse = await axios.get(`/api/cibinong/main/transaksikeluar?id=${modifiedId}`);
      ...
      if (transaksiKeluarResponse.data.length === 0) {
        const postResponse = await axios.post(`/api/cibinong/main/transaksikeluar`, postData);
        console.log(postResponse.data.message);
      } else {
        const updateResponse = await axios.put(`/api/cibinong/main/transaksikeluar`, {
    ...
    const response = await axios.put(`/api/cibinong/main/pasien?id=${selectedPasien.id}`, {
      ...editData,
      tanggal: isoTanggal,
    });
  ...
  const handleDelete = async (id) => {
    ...
    if (isConfirmed) {
      try {
        const response = await axios.delete(`/api/cibinong/main/pasien?id=${id}`);
        console.log(response.data.message);
  ...
  useEffect(() => {
    const fetchMetodeSunatOptions = async () => {
      try {
        const response = await axios.get('/api/cibinong/main/metode-sunat');
  ...
  useEffect(() => {
    const fetchMetodeBiusOptions = async () => {
      try {
        const response = await axios.get('/api/cibinong/main/metode-bius');
  ...
  const handleDateChange = async (event) => {
    ...
    try {
    const response = await axios.get(`/api/cibinong/main/available-times?tanggal=${selectedDateString}`);
  ...
  const handleFormSubmit = async (event) => {
    ...
    try {
      await axios.post('/api/cibinong/main/pasien', data);
