...
const TransaksiKeluar = () => {
  ...
  const fetchFilteredData = async () => {
    try {
      const response = await axios.get(`/api/cibinong/main/transaksi-keluar`, {
        params: {
  ...
  useEffect(() => {
    axios.get('/api/cibinong/main/get-options?action=kategori')
      .then(response => {
  ...
  useEffect(() => {
    if (selectedKategori) {
      axios.get(`/api/cibinong/main/get-options?action=item&kategori=${selectedKategori}`)
        .then(response => {
  ...
  const handleSubmit = (event) => {
    ...
    if (selectedKategori && selectedItem && tanggalTransaksi && qty && penerima) {
      ...
      axios.post('/api/cibinong/main/transaksi-keluar', data)
        .then(response => {
          ...
          if (penerima === 'Ciledug') {
            ...
            axios.post('/api/cibinong/ciledug/submit-transaksi-masuk-ciledug', transaksiMasukData)
              .then(response => {
            ...
            axios.post('/api/cibinong/pamulang/submit-transaksi-masuk-pamulang', transaksiMasukData)
              .then(response => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await axios.get('/api/cibinong/main/bgst'); // Sesuaikan dengan URL API yang Anda buat
    ...
    if (!result.destination) {
      ...
      if (itemToDelete) {
        if (window.confirm(`Are you sure you want to delete ${findItemNameById(parseInt(itemToDelete.barang_id), bgst)}?`)) {
          ...
          axios.delete(`/api/cibinong/main/transaksi-keluar?id=${itemId}`)
            .then(response => {
              ...
              if (itemToDelete.penerima === 'Ciledug') {
                ...
                const deleteParams = {
                  ...
                  pengirim: 'Cibinong' // Ubah sesuai dengan pengirim yang diinginkan
                };
  
                // Perform delete operation using API untuk menghapus data berdasarkan tanggal_transaksi, barang_id, qty, dan pengirim
                axios.delete('/api/cibinong/ciledug/delete-transaksi-masuk-ciledug', { params: deleteParams })
                  .then(deleteResponse => { // Data berhasil dihapus, lakukan apa yang perlu dilakukan
                    console.log('Data from ciledug_db deleted:', deleteResponse.data);
                  })
                  .catch(deleteError => {
                    console.error('Error deleting data from ciledug_db:', deleteError);
                  });
              }
              ...
              if (itemToDelete.penerima === 'Pamulang') {
                ...
                // Perform delete operation using API untuk menghapus data berdasarkan tanggal_transaksi, barang_id, qty, dan pengirim
                axios.delete('/api/cibinong/pamulang/delete-transaksi-masuk-pamulang', { params: deleteParams })
                  .then(deleteResponse => { // Data berhasil dihapus, lakukan apa yang perlu dilakukan
                    console.log('Data from ciledug_db deleted:', deleteResponse.data);
                  })
                  .catch(deleteError => {
  ...
  const fetchDetailedData = async () => {
    try {
      const response = await axios.get(`/api/cibinong/main/get-detailed-transaksi-keluar`, {
        params: {
          startDate: selectedStartDate,
          endDate: selectedEndDate,
        },
  ...
  const handleSimplifyClick = () => {
    ...
    axios.delete('/api/cibinong/main/simplify-transaksi-keluar')
      .then(response => {
