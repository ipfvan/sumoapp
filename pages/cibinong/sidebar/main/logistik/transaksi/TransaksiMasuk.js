...
const TransaksiMasuk = () => {
  ...
  const fetchFilteredData = async () => {
    try {
      const response = await axios.get(`/api/cibinong/main/transaksi-masuk`, {
        params: {
  ...
  const handleSubmit = (event) => {
    ...
    if (selectedKategori && selectedItem && tanggalTransaksi && qty && pengirim) {
      ...
      axios.post('/api/cibinong/main/transaksi-masuk', data)
        .then(response => {
    ...
    if (!result.destination) {
      ...
      if (itemToDelete) {
        if (itemToDelete.penerima === "Ciledug") {
          ...
          axios.get(`/api/cibinong/ciledug/delete-transaksi-masuk-ciledug`, {
            params: {
          ...
          .then(ciledugData => {
            if (ciledugData.length > 0) {
              ...
              axios.delete(`/api/cibinong/ciledug/delete-transaksi-masuk-ciledug?id=${ciledugItemId}`)
                .then(response => {
  ...
  const fetchDetailedData = async () => {
    try {
      const response = await axios.get(`/api/cibinong/main/get-detailed-transaksi-masuk`, {
        params: {
          startDate: selectedStartDate,
          endDate: selectedEndDate,
        },
  ...
  const handleSimplifyClick = () => {
    ...
    axios.delete('/api/cibinong/main/simplify-transaksi-masuk')
      .then(response => {
