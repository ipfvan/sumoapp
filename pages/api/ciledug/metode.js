...
export default async (req, res) => {
  if (req.method === 'GET') {
    try {
      const result = await executeCiledug('SELECT * FROM metode');
    ...
    try {
      await executeCiledug('DELETE FROM metode WHERE id = ?', [id]);
    ...
    try {
      await executeCiledug('INSERT INTO metode (nama_metode, harga, fee_dr) VALUES (?, ?, ?)', [nama_metode, harga, fee_dr]);
    ...
    try {
      await executeCiledug(updateQuery, [updatedItem, itemId]);
