...
export default async function handler(req, res) {
  ...
  try {
    ...
    const configData = await executeCibinong(getConfigQuery);
    ...
    const specialConfigData = await executeCibinong(getSpecialConfigQuery, [tanggal]);
    ...
    const latestTimeResult = await executeCibinong(checkDateQuery, [tanggal]);
