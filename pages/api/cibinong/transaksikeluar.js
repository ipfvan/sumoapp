...
export default async function handler(req, res) {
  try {
    if (req.method === 'POST') {
      ...
      await executeCibinong(insertQuery, [id, barang_id, tanggal_transaksi, qty, penerima]);
      ...
      const results = await executeCibinong(selectQuery, [id]);
      res.status(200).json(results);
    } else if (req.method === 'PUT') {
      const { id, barang_id, tanggal_transaksi, qty, penerima } = req.body;
      const updateQuery = 'UPDATE transaksi_keluar SET barang_id = ?, tanggal_transaksi = ?, qty = ?, penerima = ? WHERE id = ?';
      const result = await executeCibinong(updateQuery, [barang_id, tanggal_transaksi, qty, penerima, id]);
