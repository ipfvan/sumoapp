...
export default async (req, res) => {
  if (req.method === 'GET') {
    try {
      const result = await executeCibinong('SELECT * FROM metode');
    ...
    try {
      await executeCibinong('DELETE FROM metode WHERE id = ?', [id]);
    ...
    try {
      await executeCibinong('INSERT INTO metode (nama_metode, harga, fee_dr) VALUES (?, ?, ?)', [nama_metode, harga, fee_dr]);
    ...
    try {
      await executeCibinong(updateQuery, [updatedItem, itemId]);
