...
const Celana = () => {
  ...
  useEffect(() => {
    ...
    fetchList('Celana', setCelanaList);
  }, []);
  ...
  const handleDelete = (id) => {
    ...
    if (confirmDelete) {
      axios.delete(`/api/cibinong/main/barang?id=${id}`)
        .then(response => {
          console.log(response.data.message);
          const updatedList = celanaList.filter(item => item.id !== id);
          setCelanaList(updatedList);
        })
        .catch(error => {
  ...
  const handleAddNewItem = () => {
    ...
    axios.post('/api/cibinong/main/barang', { kategori: 'Celana', item: newItemName })
      .then(response => {
        console.log(response.data.message);
        fetchList('Celana', setCelanaList);
        setNewItemName('');
  ...
  return (
        <div className="container mx-auto p-4">
          <h1 className="text-2xl font-bold mb-4">Celana</h1>
                ...
                <Droppable droppableId="itemList" type="ITEM">
                  {(provided) => (
                    <tbody {...provided.droppableProps} ref={provided.innerRef}>
                      {celanaList.map((celana, index) => (
                        <Draggable
                          key={celana.id.toString()}
                          draggableId={celana.id.toString()}
                          index={index}
                        >
                          {(provided) => (
                            <tr
                            ...
                            >
                              <td className="border border-gray-400 px-4 py-2 text-left">
                                {editedItems[celana.id] !== undefined ? (
                                  <input
                                    type="text"
                                    value={editedItems[celana.id]}
                                    onChange={(e) => {
                                      setEditedItems({
                                        ...editedItems,
                                        [celana.id]: e.target.value,
                                      });
                                    }}
                                    onBlur={() => handleEdit(celana.id, "item")}
                                  />
                                ) : (
                                  <span
                                    onClick={() => {
                              ...
                              <td className="border border-gray-400 px-4 py-2">
                                {editedQty[celana.id] !== undefined ? (
                                  <input
                                    type="text"
                                    value={editedQty[celana.id]}
                                    onChange={(e) => {
                                      setEditedQty({
                                        ...editedQty,
                                        [celana.id]: e.target.value,
                                    ...
                                    onBlur={() => handleEdit(celana.id, "qty")}
                                  />
                                ) : (
                                  <span
                                    onClick={() => {
                              ...
                              <td className="border border-gray-400 px-4 py-2">
  {editedSatuan[celana.id] !== undefined ? (
    <input
      type="text"
      value={editedSatuan[celana.id]}
      onChange={(e) => {
        setEditedSatuan({
          ...editedSatuan,
          [celana.id]: e.target.value,
      ...
      onBlur={() => handleEdit(celana.id, "satuan")}
    />
  ) : (
    <span
      onClick={() => {
...
<td className="border border-gray-400 px-4 py-2">
  {editedExpired[celana.id] !== undefined ? (
    <input
      type="text"
      value={(editedExpired[celana.id])}
      onChange={(e) => {
        setEditedExpired({
          ...editedExpired,
          [celana.id]: e.target.value,
      ...
      onBlur={() => handleEdit(celana.id, "expired")}
    />
  ) : (
    <span
      onClick={() => {
        setEditedExpired({
          ...editedExpired,
          [celana.id]: formatTanggal(celana.expired),
        });
      }}
    >
      {formatTanggal(celana.expired)}
    </span>
  )}
</td>
                          ...
                          <td className="border border-gray-400">
                            <button
                              ...
                              onClick={() => handleDelete(celana.id)}
                            >
                              Delete
                            </button>
                          </td>
