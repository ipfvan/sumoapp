...
const JamUmum = () => {
  ...
  useEffect(() => {
    ...
    axios.get('/api/ciledug/main/configurations') // Gantilah '/api/ciledug/main/endpoint' dengan URL yang sesuai
      .then((response) => {
...
const handleUpdateClick = () => {
  ...
  axios.post('/api/ciledug/main/configurations', formData)
    .then((response) => {
      console.log('Configuration updated successfully');
      setEditing(false);

      // Lakukan fetch ulang untuk mendapatkan data yang paling mutakhir
      axios.get('/api/ciledug/main/configurations')
        .then((response) => {
          setJam(response.data);
        })
        .catch((error) => {
