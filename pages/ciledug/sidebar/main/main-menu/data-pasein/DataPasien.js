...
export default function DataPasien()  {
  ...
  useEffect(() => {
    const fetchOptions = async (kategori, setOptions) => {
      try {
        const response = await axios.get(`/api/ciledug/main/items-by-category?kategori=${kategori}`);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('/api/ciledug/main/pasien');
...
const handleEditSubmit = async (e) => {
  ...
  try {
    ...
    for (const caseItem of cases) {
      ...
      const transaksiKeluarResponse = await axios.get(`/api/ciledug/main/transaksikeluar?id=${modifiedId}`);
      ...
      if (transaksiKeluarResponse.data.length === 0) {
        const postResponse = await axios.post(`/api/ciledug/main/transaksikeluar`, postData);
        console.log(postResponse.data.message);
      } else {
        const updateResponse = await axios.put(`/api/ciledug/main/transaksikeluar`, {
    ...
    const response = await axios.put(`/api/ciledug/main/pasien?id=${selectedPasien.id}`, {
      ...editData,
      tanggal: isoTanggal,
    });
  ...
  const handleDelete = async (id) => {
    ...
    if (isConfirmed) {
      try {
        const response = await axios.delete(`/api/ciledug/main/pasien?id=${id}`);
        console.log(response.data.message);
