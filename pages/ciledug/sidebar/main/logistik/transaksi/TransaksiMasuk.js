...
const TransaksiMasuk = () => {
  ...
  const fetchFilteredData = async () => {
    try {
      const response = await axios.get(`/api/ciledug/main/transaksi-masuk`, {
        params: {
  ...
  const handleSubmit = (event) => {
    ...
    if (selectedKategori && selectedItem && tanggalTransaksi && qty && pengirim) {
      ...
      axios.post('/api/ciledug/main/transaksi-masuk', data)
        .then(response => {
    ...
    if (!result.destination) {
      ...
      if (itemToDelete) {
        if (itemToDelete.penerima === "Ciledug") {
          ...
          axios.get(`/api/ciledug/cibinong/delete-transaksi-masuk-ciledug`, {
            params: {
          ...
          .then(ciledugData => {
            if (ciledugData.length > 0) {
              ...
              axios.delete(`/api/ciledug/cibinong/delete-transaksi-masuk-ciledug?id=${ciledugItemId}`)
                .then(response => {
  ...
  const fetchDetailedData = async () => {
    try {
      const response = await axios.get(`/api/ciledug/main/get-detailed-transaksi-masuk`, {
        params: {
          startDate: selectedStartDate,
          endDate: selectedEndDate,
        },
  ...
  const handleSimplifyClick = () => {
    ...
    axios.delete('/api/ciledug/main/simplify-transaksi-masuk')
      .then(response => {
