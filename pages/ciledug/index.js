...
import HeaderTiga from './sidebar/cibinong/main-menu/HeaderTiga';
import HeaderEmpat from './sidebar/cibinong/logistik/HeaderEmpat';
...
export default function CiledugHome() {
  ...
  const loadNotifications = async () => {
    try {
      const response = await axios.get('/api/ciledug/main/notifications');
  ...
  const clearNotifications = async () => {
    try {
      await axios.delete('/api/ciledug/main/notifications');
      setNotifications([]);
  ...
  const toggleNotifications = async () => {
    ...
    if (isNewNotification && notifications.length > 0) {
      try {
        ...
        await axios.put('/api/ciledug/main/notifications/', { notificationIds });
