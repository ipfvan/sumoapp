...
const RegistrationPamulang = () => {
  ...
  const handleAgreeTerms = async (event) => {
    ...
    try {
      ...
      socket.emit('playAudio3');
      socket.emit('newPatientRegistered', data);
  
      try {
        await axios.post('/api/pamulang/main/notifications', {
          nama: data.nama,
          jam: data.jam,
  ...
  return (
<div>
<h2 className="text-2xl font-bold mb-2 text-customPurple">Form Pendaftaran Cabang Pamulang</h2>
...
export default RegistrationPamulang;
