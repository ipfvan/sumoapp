...
const JamUmum = () => {
  ...
  useEffect(() => {
    ...
    axios.get('/api/pamulang/configurations') // Gantilah '/api/pamulang/endpoint' dengan URL yang sesuai
      .then((response) => {
...
const handleUpdateClick = () => {
  ...
  axios.post('/api/pamulang/configurations', formData)
    .then((response) => {
      console.log('Configuration updated successfully');
      setEditing(false);

      // Lakukan fetch ulang untuk mendapatkan data yang paling mutakhir
      axios.get('/api/pamulang/configurations')
        .then((response) => {
          setJam(response.data);
        })
        .catch((error) => {
