...
const Dokter = () => {
  ...
  const fetchMetodeData = async () => {
    try {
      const response = await axios.get(`/api/pamulang/dokter`);
  ...
  const handleAddData = () => {
    ...
    axios
      .post('/api/pamulang/dokter', newData) // Replace with your API endpoint
      .then((response) => {
        setIsAdding(false);
  ...
  const handleDragEnd = (result) => {
    ...
    axios.post('/api/pamulang/update-order-dokter', { updatedList })
      .then(response => {
        console.log(response.data.message);
      })
      .catch(error => {
  ...
  const handleEdit = (id, column) => {
    ...
    if (editedValue !== undefined) {
      axios.put('/api/pamulang/dokter', { id, column, updatedItem: editedValue })
        .then(response => {
          console.log(response.data.message);
  ...
  const handleDelete = (id) => {
    ...
    if (shouldDelete) {
      ...
      axios.delete(`/api/pamulang/dokter?id=${id}`)
        .then(() => {
  ...
  {editedAlamat[metode.id] !== undefined ? (
    ...
    >
      ...
      <div className="mb-4 p-4">
        ...
        <button
          className="bg-purple-900 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-4"
          onClick={handleAdd}
        >
          Add Data
        </button>
