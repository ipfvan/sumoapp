...
const MetodeBack = () => {
  ...
  const fetchMetodeData = async () => {
    try {
      const response = await axios.get(`/api/pamulang/metode`);
  ...
  const handleAddData = () => {
    ...
    axios
      .post('/api/pamulang/metode', newData) // Replace with your API endpoint
      .then((response) => {
        setIsAdding(false);
  ...
  const handleDragEnd = (result) => {
    ...
    axios.post('/api/pamulang/update-order-metode', { updatedList })
      .then(response => {
        console.log(response.data.message);
      })
      .catch(error => {
  ...
  const handleEdit = (id, column) => {
    ...
    if (editedValue !== undefined) {
      axios.put('/api/pamulang/metode', { id, column, updatedItem: editedValue })
        .then(response => {
          console.log(response.data.message);
  ...
  const handleDelete = (id) => {
    ...
    if (shouldDelete) {
      ...
      axios.delete(`/api/pamulang/metode?id=${id}`)
        .then(() => {
