...
export default function DataPasien()  {
  ...
  useEffect(() => {
    const fetchOptions = async (kategori, setOptions) => {
      try {
        const response = await axios.get(`/api/pamulang/items-by-category?kategori=${kategori}`);
...
if (pamulangauthenticated) {
  ...
  try {
    ...
    for (const caseItem of cases) {
      ...
      const transaksiKeluarResponse = await axios.get(`/api/pamulang/transaksikeluar?id=${modifiedId}`);
      ...
      if (transaksiKeluarResponse.data.length === 0) {
        const postResponse = await axios.post(`/api/pamulang/transaksikeluar`, postData);
        console.log(postResponse.data.message);
      } else {
        const updateResponse = await axios.put(`/api/pamulang/transaksikeluar`, {
    ...
    const response = await axios.put(`/api/pamulang/pasien?id=${selectedPasien.id}`, {
      ...editData,
      tanggal: isoTanggal,
    });
...
if (pamulangauthenticated) {  
    if (isConfirmed) {
      try {
        const response = await axios.delete(`/api/pamulang/pasien?id=${id}`);
        console.log(response.data.message);
  ...
  const fetchFilteredData = async () => {
    try {
      const response = await axios.get(`/api/pamulang/pasien`, {
        params: {
...
if (pamulangauthenticated) {
   // Dapatkan nilai dari input-form sesuai dengan nama kolom di database
    ...
    try {
      await axios.post('/api/pamulang/pasien', data);
