...
export default function DataPasien()  {
  ...
  useEffect(() => {
    const fetchOptions = async (kategori, setOptions) => {
      try {
        const response = await axios.get(`/api/ciledug/items-by-category?kategori=${kategori}`);
...
if (ciledugauthenticated) {
  ...
  try {
    ...
    for (const caseItem of cases) {
      ...
      const transaksiKeluarResponse = await axios.get(`/api/ciledug/transaksikeluar?id=${modifiedId}`);
      ...
      if (transaksiKeluarResponse.data.length === 0) {
        const postResponse = await axios.post(`/api/ciledug/transaksikeluar`, postData);
        console.log(postResponse.data.message);
      } else {
        const updateResponse = await axios.put(`/api/ciledug/transaksikeluar`, {
    ...
    const response = await axios.put(`/api/ciledug/pasien?id=${selectedPasien.id}`, {
      ...editData,
      tanggal: isoTanggal,
    });
...
if (ciledugauthenticated) {  
    if (isConfirmed) {
      try {
        const response = await axios.delete(`/api/ciledug/pasien?id=${id}`);
        console.log(response.data.message);
  ...
  const fetchFilteredData = async () => {
    try {
      const response = await axios.get(`/api/ciledug/pasien`, {
        params: {
...
if (ciledugauthenticated) {
   // Dapatkan nilai dari input-form sesuai dengan nama kolom di database
    ...
    try {
      await axios.post('/api/ciledug/pasien', data);
  ...
  return (
    ...
    <div className="rounded-xl min-h-screen bg-gray-100">
         <div>
    {isLoading ? (
