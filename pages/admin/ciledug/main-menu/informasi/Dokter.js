...
const Dokter = () => {
  ...
  const fetchMetodeData = async () => {
    try {
      const response = await axios.get(`/api/ciledug/dokter`);
  ...
  const handleAddData = () => {
    ...
    axios
      .post('/api/ciledug/dokter', newData) // Replace with your API endpoint
      .then((response) => {
        setIsAdding(false);
  ...
  const handleDragEnd = (result) => {
    ...
    axios.post('/api/ciledug/update-order-dokter', { updatedList })
      .then(response => {
        console.log(response.data.message);
      })
      .catch(error => {
  ...
  const handleEdit = (id, column) => {
    ...
    if (editedValue !== undefined) {
      axios.put('/api/ciledug/dokter', { id, column, updatedItem: editedValue })
        .then(response => {
          console.log(response.data.message);
  ...
  const handleDelete = (id) => {
    ...
    if (shouldDelete) {
      ...
      axios.delete(`/api/ciledug/dokter?id=${id}`)
        .then(() => {
