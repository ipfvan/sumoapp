...
const TransaksiMasuk = () => {
  ...
  const fetchFilteredData = async () => {
    try {
      const response = await axios.get(`/api/cibinong/transaksi-masuk`, {
        params: {
...
if (cibinongauthenticated) {
    ...
    if (selectedKategori && selectedItem && tanggalTransaksi && qty && pengirim) {
      ...
      axios.post('/api/cibinong/transaksi-masuk', data)
        .then(response => {
    ...
    if (!result.destination) {
      ...
      if (itemToDelete) {
        ...
        if (itemToDelete.penerima === "Ciledug") {
          ...
          axios.get(`/api/cibinong/delete-transaksi-masuk-ciledug`, {
            params: {
          ...
          .then(ciledugData => {
            if (ciledugData.length > 0) {
              ...
              axios.delete(`/api/cibinong/delete-transaksi-masuk-ciledug?id=${ciledugItemId}`)
                .then(response => {
  ...
  const fetchDetailedData = async () => {
    try {
      const response = await axios.get(`/api/cibinong/get-detailed-transaksi-masuk`, {
        params: {
          startDate: selectedStartDate,
          endDate: selectedEndDate,
        },
  ...
  const handleSimplifyClick = () => {
    ...
    axios.delete('/api/cibinong/simplify-transaksi-masuk')
      .then(response => {
