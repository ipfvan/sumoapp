...
const MetodeBack = () => {
  ...
  const fetchMetodeData = async () => {
    try {
      const response = await axios.get(`/api/cibinong/metode`);
  ...
  const handleAddData = () => {
    ...
    axios
      .post('/api/cibinong/metode', newData) // Replace with your API endpoint
      .then((response) => {
        setIsAdding(false);
  ...
  const handleDragEnd = (result) => {
    ...
    axios.post('/api/cibinong/update-order-metode', { updatedList })
      .then(response => {
        console.log(response.data.message);
      })
      .catch(error => {
  ...
  const handleEdit = (id, column) => {
    ...
    if (editedValue !== undefined) {
      axios.put('/api/cibinong/metode', { id, column, updatedItem: editedValue })
        .then(response => {
          console.log(response.data.message);
  ...
  const handleDelete = (id) => {
    ...
    if (shouldDelete) {
      ...
      axios.delete(`/api/cibinong/metode?id=${id}`)
        .then(() => {
