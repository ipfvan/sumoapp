...
const Dokter = () => {
  ...
  const fetchMetodeData = async () => {
    try {
      const response = await axios.get(`/api/cibinong/dokter`);
  ...
  const handleAddData = () => {
    ...
    axios
      .post('/api/cibinong/dokter', newData) // Replace with your API endpoint
      .then((response) => {
        setIsAdding(false);
  ...
  const handleDragEnd = (result) => {
    ...
    axios.post('/api/cibinong/update-order-dokter', { updatedList })
      .then(response => {
        console.log(response.data.message);
      })
      .catch(error => {
  ...
  const handleEdit = (id, column) => {
    ...
    if (editedValue !== undefined) {
      axios.put('/api/cibinong/dokter', { id, column, updatedItem: editedValue })
        .then(response => {
          console.log(response.data.message);
  ...
  const handleDelete = (id) => {
    ...
    if (shouldDelete) {
      ...
      axios.delete(`/api/cibinong/dokter?id=${id}`)
        .then(() => {
